"""通用 DyAD++ (任意 brand): A1 cyclical-KL / A2 robust里程归一化 / A3 多信号(重建+隐空间马氏)。
用法: python repro/improve.py brand2 --fold 0 [--epochs N]"""
...
def run(brand, fold, epochs, outdir):
    ...
    hp = B.BRANDS[brand]['hp']; epochs = epochs or max(30, hp['epochs']*6)   # 默认充分训练
    ...
    print(f"[{brand}++ f{fold}] train={tr.sum()} test={te.sum()} ep={epochs}")
    nrm = Norm(Xtr[:200]); Xtr_n = nrm(Xtr).astype(np.float32); Xte_n = nrm(Xte).astype(np.float32)
    q5, q95 = np.percentile(miltr, 5), np.percentile(miltr, 95)                # [A2]
    ...
    spe = max(1, Ntr//hp['batch']); period = spe*max(2, epochs//4); step = 0     # [A1] 4 个 KL 循环
    for ep in range(epochs):
        ...
        for s in range(0, Ntr, hp['batch']):
            ...
            klw = hp['anneal0']*min(1.0, (step % period)/(period/2)); lbl = mse(mp.squeeze(), mb)
    ...
    mu = Ztr.mean(0); cov = np.cov(Ztr.T) + 1e-4*np.eye(L); inv = np.linalg.inv(cov)   # [A3]
    d = Zte-mu; maha = np.einsum('ij,jk,ik->i', d, inv, d).astype(np.float32)
    ...
    print(f"[{brand}++ f{fold}] saved {outdir}/seg_fold{fold}.csv")
...
if __name__ == '__main__':
    ...
    a = ap.parse_args(); run(a.brand, a.fold, a.epochs, a.outdir or f'repro/out/{a.brand}_pp')
