"""R1: 修复版 DyAD++ — brand1 noise降低 + KL预热, brand2 收缩协方差, 通用GMM准备。
用法: python repro/method_r1_stable.py brand1 --fold 0 [--epochs N]"""
...
def run(brand, fold, epochs, outdir):
    ...
    # [R1-Fix1] brand1: noise 1.0→0.01 防 latent 爆炸
    ...
    print(f"[{brand}-r1 f{fold}] train={tr.sum()}({len(train_cars)}车) test={te.sum()}({len(test_cars)}车) ep={epochs} noise={hp['noise']}")
    ...
    warmup_steps = (epochs//4)*spe  # [R1-Fix2] brand1 前 epochs/4 用线性KL预热
    for ep in range(epochs):
        ...
        for s in range(0, Ntr, hp['batch']):
            ...
            # [R1-Fix2] brand1: 线性预热 → 循环; 其他brand: 全程循环
    ...
    # [R1-Fix3] brand2: LedoitWolf收缩协方差, 其余brand也用(比朴素更稳)
    ...
    print(f"[{brand}-r1 f{fold}] saved {outdir}/seg_fold{fold}.csv")
...
if __name__ == '__main__':
    ...
    a = ap.parse_args(); run(a.brand, a.fold, a.epochs, a.outdir or f'repro/out/{a.brand}_r1')
