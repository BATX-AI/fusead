"""R2b: R2(GMM) + brand1全程线性KL。brand2/3保持原R2训练逻辑。
用法: python repro/method_r2b_linear.py brand1 --fold 0"""
...
def run(brand, fold, epochs, outdir):
    ...
    kl_type = 'linear' if brand == 'brand1' else 'cyclical'
    print(f"[{brand}-r2b f{fold}] train={tr.sum()}({len(train_cars)}车) test={te.sum()}({len(test_cars)}车) ep={epochs} noise={hp['noise']} KL={kl_type}", flush=True)
    ...
    for ep in range(epochs):
        ...
        for s in range(0, Ntr, hp['batch']):
            ...
            if brand == 'brand1':
                klw = hp['anneal0']*min(1.0, step/hp['x0'])  # 全程线性
        ...
        sch.step()
        if (ep+1) % 5 == 0 or ep == 0: print(f"  ep{ep+1}/{epochs} loss={tot/spe:.4f}", flush=True)
    model.eval()
    def enc(Xn):
    ...
    print(f"  GMM k={best_k} (max={max_k}, cars={n_cars})", flush=True)
    gmm = GaussianMixture(n_components=best_k, covariance_type='full', random_state=0, reg_covar=1e-4).fit(Ztr)
    gmm_nll_tr = -gmm.score_samples(Ztr)
    gmm_nll_te = -gmm.score_samples(Zte)
    ...
    print(f"[{brand}-r2b f{fold}] saved {outdir}/seg_fold{fold}.csv", flush=True)
...
if __name__ == '__main__':
    ...
    a = ap.parse_args(); run(a.brand, a.fold, a.epochs, a.outdir or f'repro/out/{a.brand}_r2b')
