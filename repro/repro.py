"""通用 DyAD 忠实复现 (任意 brand)。用法: python repro/repro.py brand2 --fold 0 [--epochs N]"""
...
def run(brand, fold, epochs, outdir):
    ...
    hp = B.BRANDS[brand]['hp']; epochs = epochs or hp['epochs']
    ...
    print(f"[{brand} f{fold}] train={tr.sum()}({len(train_cars)}车) test={te.sum()}({len(test_cars)}车,{len(ood)}故障) ep={epochs} hp={hp['hidden']}h/{hp['latent']}z")
    ...
    for ep in range(epochs):
        ...
        sch.step(); print(f"  ep{ep+1}/{epochs} loss={tot/max(1,Ntr//hp['batch']):.4f}")
...
if __name__ == '__main__':
    ...
    a = ap.parse_args(); run(a.brand, a.fold, a.epochs, a.outdir or f'repro/out/{a.brand}_base')
