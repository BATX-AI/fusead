"""P1 快速扫参: 只用 LedoitWolf (快), 不加 GMM。
用法: python repro/method_p1_fast.py brand1 --noise 1.0 --epochs 3 --fold 0"""
...
def run(brand, fold, epochs, outdir, noise_override, hidden_override, latent_override):
    ...
    print(f"[{brand} f{fold}] ep={epochs} noise={hp['noise']} hidden={hp['hidden']} L={L}", flush=True)
    ...
    for ep in range(epochs):
        ...
        sch.step()
    # 快速推理: 只输出 rec_error + maha, 不跑 GMM
    model.eval()
    def enc(Xn):
    ...
    print(f"[{brand} f{fold}] saved {outdir}/seg_fold{fold}.csv", flush=True)
