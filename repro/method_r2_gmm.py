"""R2: GMM隐空间密度 — 复制R1稳定训练, 将单高斯马氏→GMM负对数似然。
用法: python repro/method_r2_gmm.py brand1 --fold 0 [--epochs N]"""
...
def run(brand, fold, epochs, outdir):
    ...
    print(f"[{brand}-r2 f{fold}] train={tr.sum()}({len(train_cars)}车) test={te.sum()}({len(test_cars)}车) ep={epochs} noise={hp['noise']}")
    ...
    print(f"  GMM k={best_k} (max={max_k}, cars={n_cars})")
    ...
    print(f"[{brand}-r2 f{fold}] saved {outdir}/seg_fold{fold}.csv")
...
if __name__ == '__main__':
    ...
    a = ap.parse_args(); run(a.brand, a.fold, a.epochs, a.outdir or f'repro/out/{a.brand}_r2')
